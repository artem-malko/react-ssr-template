...
describe('config / utils', () => {
  describe('parseEnvParams', () => {
    it(`Replace number value in initial config
      ...
      expect(parseEnvParams({ port: 3000 }, { SERVER_PORT: '5000' }, /^server_/i)).to.deep.eq({
        port: 5000,
      });
    });
    ...
    it(`Do not replace number value in initial config
      ...
      expect(parseEnvParams({ port: 3000 }, { SERVER_PORT: 'ase' }, /^server_/i)).to.deep.eq({
        port: 3000,
      });
    });
    ...
    it(`Do not replace number value in initial config
      ...
      expect(parseEnvParams({ port: 3000 }, { SERVER_PORT: '5000' }, /^client_/i)).to.deep.eq({
        port: 3000,
      });
    });
    ...
    it(`Do not replace any value in initial config
        if there is no any replacement in env params`, () => {
      expect(parseEnvParams({ port: 3000 }, { CLIENT_LABEL: 'label' }, /^client_/i)).to.deep.eq({
        port: 3000,
      });
    });
    ...
    it(`Replace string value in initial config
      ...
      expect(parseEnvParams({ url: 'domain.com' }, { CLIENT_URL: 'c.d.com' }, /^client_/i)).to.deep.eq({
        url: 'c.d.com',
      });
    });
    ...
    it(`Replace boolean value (true) in initial config
        if there is a correct replacement in env params`, () => {
      expect(parseEnvParams({ sendLogs: false }, { CLIENT_SEND_LOGS: 'true' }, /^client_/i)).to.deep.eq({
        sendLogs: true,
      });
    });
    ...
    it(`Replace boolean value (false) in initial config
        if there is a correct replacement in env params`, () => {
      expect(parseEnvParams({ sendLogs: true }, { CLIENT_SEND_LOGS: 'false' }, /^client_/i)).to.deep.eq({
        sendLogs: false,
      });
    });
    ...
    it(`Replace values from initial config only
        ...
        (regExp is for client, env params for client)`, () => {
      expect(
        parseEnvParams({ port: 3000 }, { SERVER_PORT: '5000', PATH: 'normal' }, /^server_/i),
      ).to.deep.eq({
        port: 5000,
      });
    });
